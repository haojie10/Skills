...
def build_request_body(prompt: str, image_size: str, aspect_ratio: str = "1:1") -> bytes:
...
Environment Variables:
  ...
  GEMINI_MODEL      Model ID for image generation
    ...
    parser.add_argument("--ar", choices=["1:1", "9:16", "16:9", "3:4", "4:3"], default="1:1",
    ...
    # Auto-detect aspect ratio from prompt if not explicitly set
    aspect_ratio = args.ar
    if aspect_ratio == "1:1" and ("16:9" in args.prompt.lower() or "horizontal" in args.prompt.lower()):
        aspect_ratio = "ASPECT_RATIO_16_9"
    elif aspect_ratio == "1:1" and ("9:16" in args.prompt.lower() or "vertical" in args.prompt.lower()):
