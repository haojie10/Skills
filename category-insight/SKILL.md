...
本技能提供了一套规范的全球零售渠道品类调研与对比分析工作流，通过自动化的报告生成、Base64 图片内嵌、文件清理技术以及一键平台上传，为用户交付一份自包含、无冗余临时文件且已发布上线至平台（`https://marketgraphic.cn`）的精美 HTML 商业洞察报告。
...
3. 复制本技能预置的毛玻璃特效 HTML 模板：
   ...
   - **本地存储路径规范（强制要求）**：
     - 所有生成的报告文件统一保存在项目根目录下的 **`report/`** 目录中；
     - 品类洞察报告必须保存在 **`report/Catagory Insight/`** 文件夹中（若该文件夹不存在，脚本/Agent 必须先执行 `mkdir -p "report/Catagory Insight"` 自动创建）；
     - 最终生成的 HTML 文件按照**「品类 + 市场」**规范命名（例如 `report/Catagory Insight/swim-cap-canadian-retail-insight.html`，符合短横线命名标准）。
4. **命名与标题规范（强制要求）**：
...
2. **自动发布与真实 ID 校验逻辑（强制要求）**：
   - 该脚本会自动读取 `.env` 中的 `GTB_API_URL`（默认 `https://marketgraphic.cn`）与 `AGENT_API_KEY`。
...
- **存储路径（强制要求）**：报告必须保存在 `report/Catagory Insight/` 目录下，若路径不存在则自动创建，杜绝在根目录或临时目录随意散落文件。
